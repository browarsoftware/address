...
[t_v_rho, T_h_arr, d_help, flows] = read_data('data/Wloclawek/t_v_rho.csv', 'data/Wloclawek/T_h_arr.csv',
                                              'data/Wloclawek/d.csv', 'data/Wloclawek/flows.csv')
...
#  average depth of the entry cross section [m], reservoir input
#h_in = 1.5
h_in = 4
...
dt_string = 'Wloclawek/' + str(now.strftime("%d-%m-%Y %H-%M-%S"))
import os
...
# save setup
setup_file_name = 'results/' + dt_string + '/MSE_setup.csv'
...
results_file_name = 'results/' + dt_string + '/MSE_results.csv'
with open(results_file_name, 'a') as fd:
    fd.write("flow_id,s,exp(mu),error\n")

# file with solutions
solution_file_name = 'results/' + dt_string + '/MSE_solution.csv'
with open(solution_file_name, 'a') as fd:
    fd.write("Y,y_pred\n")

for flow_id in range(len(flows)):
    # file with optimization results
    optimization_file_name = 'results/' + dt_string + '/MSE_optimization_' + str(flow_id) + '.csv'

    Y = flows[flow_id]
    my_T_h_init = T_h_arr[flow_id]
    iter_count = 0
    def myCallback_MSE(xk, convergence):
        global iter_count
        global distribution_sample_size
        iter_count = iter_count + 1
        y_pred = draw_distribution(xk[0], np.exp(xk[1]), distribution_sample_size)
        print("iter=" +str(iter_count) + "xk=[" + str(xk[0]) + "," + str(np.exp(xk[1])) + "] , error=" + str(error_function_MSE(y_pred, Y)))
        with open(optimization_file_name, 'a') as fd:
            if iter_count == 1:
                fd.write("iter,s,exp(mu),error\n")
            fd.write(str(iter_count) + "," + str(xk[0]) + "," + str(np.exp(xk[1])) + "," + str(error_function_MSE(y_pred, Y)) + "\n")

    # starting point of optimization
    x0 = [np.log(my_T_h_init), np.log(my_T_h_init)]
    # optimize MSE
    result = differential_evolution(func=sum_of_squares, bounds=bounds, args=(Y), callback=myCallback_MSE,
    ...
    with open(results_file_name, 'a') as fd:
        ...
        print(y_pred)
        print(Y)
        print(error_function_MSE(y_pred, Y))

        fd.write(str(flow_id) + ',' + str(result.x[0]) + ',' + str(np.exp(result.x[1])) + ',' + str(error_function_MSE(y_pred, Y)) + '\n')
