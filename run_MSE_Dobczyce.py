...
h_in_help = [1.5, 1.5, 1.5, 1.5, 2.6, 2.6, 3]
# avarage flow depth odirectly upstream of the dam (outflow) [m]
h_out_help = [28, 28, 28, 28, 30.6, 30.6, 31.5]
h_out = h_out_help[0]
...
# save setup
setup_file_name = 'results/Dobczyce/' + dt_string + '/MSE_setup.csv'
...
results_file_name = 'results/Dobczyce/' + dt_string + '/MSE_results.csv'
with open(results_file_name, 'a') as fd:
    fd.write("flow_id,s,exp(mu),error\n")

# file with solutions
solution_file_name = 'results/Dobczyce/' + dt_string + '/MSE_solution.csv'
...
for flow_id in range(len(flows)):
    ...
    optimization_file_name = 'results/Dobczyce/' + dt_string + '/MSE_optimization_' + str(flow_id) + '.csv'
